...
public class NodoResource {
	...
	public Response crearNodo(Nodo nodo) throws SQLException, Exception{
	...
	@GET
	@Produces({ MediaType.APPLICATION_JSON } )
	public Response darNodos() throws SQLException, Exception{
		RedetekApiTM tm = new RedetekApiTM(getPath());
		try {
			
			return Response.status( 200 ).entity( tm.darNodosPor(0, "hola") ).build();	
		}catch( Exception e )
		{
			return Response.status( 500 ).entity( doErrorMessage( e ) ).build( );
		}
